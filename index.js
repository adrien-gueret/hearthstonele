(function () {
  ...
  var firstClueFlavorTextContainer = document.getElementById(
    "firstClueFlavorTextContainer"
  );
  ...
  function getDefaultClue() {
    ...
    return ["IMAGE_1", "FLAVOR_TEXT", "NONE"].indexOf(storedDefaultClue) === -1
      ? "FLAVOR_TEXT"
      : storedDefaultClue;
  }
  ...
  function insertTypeClue(minionTypeSlug, minionTypeName) {
  ...
  function useClue(clue, client, preventClueDamages) {
    ...
    try {
      hasUsedAllClues = currentGame.useClue(
        clue,
        preventClueDamages,
        client.cardSetGroup
      );
    ...
    switch (clue) {
      ...
      case "TEXT":
        ...
        var shuffledWords = reorderedWords.map((word) =>
          word.split("").sort(shuffle).join("")
        );

        var finalText = shuffledWords.join(" ");

        flavorText.innerHTML =
          finalText[0].toUpperCase() + finalText.substring(1) + ".";
        flavorText.style.display = "block";
        break;

      case "INITIALS":
        var censuredName = currentGame.cardToGuess.name
          .split(" ")
      ...
      case "CHANGE":
        ...
        var changeCardImageContainer =
          document.getElementById("changeCardImage");
      ...
      case "RARITY":
        client
          .getRarityName(currentGame.cardToGuess.rarityId)
          .then(function (rarityName) {
            var clue = document.createElement("span");
            clue.className = "clue";
            ...
            icon.style.backgroundPosition =
              (currentGame.cardToGuess.rarityId - 1) * -27 + "px 0";
            clue.appendChild(icon);
      ...
      case "EXPANSION":
        client
          .getCardSet(currentGame.cardToGuess.cardSetId)
          .then(function (cardSet) {
            ...
            icon.style.backgroundImage =
              "url(./images/sets/" + cardSet.slug + ".svg)";
            clue.appendChild(icon);
      ...
      case "IMAGE_1":
        Canvas.extractCardIllustration(currentGame.cardToGuess).then(function (
          canvasIllustration
        ) {
      ...
      case "TYPE":
        client
          .getMinionTypes(currentGame.cardToGuess)
          .then(function (minionTypes) {
            if (!minionTypes.length) {
              insertTypeClue("typeless", translate("typeless"));
              return;
            }

            minionTypes.forEach(function (minionType) {
              insertTypeClue(minionType.slug, minionType.name);
            });
          });
        break;
  ...
  function onGameEnd() {
    ...
    document.getElementById("gameEndScore").innerHTML = translate(
      "gameEndScore",
      currentGame.foundCards.length
    );
  ...
  function translateUI(client) {
    [
      ...
      "errorModalButton",
      "hasBeenRestored",
      "cancelHeroPowerButton",
      "heroPowerDetailsTitle",
      "heroPowerDetailsDescription",
      "heroPowerChoice1_Title",
      "heroPowerChoice2_Title",
      "heroPowerChoice3_Title",
      "heroPowerChoice4_Title",
      "heroPowerChoice5_Title",
      "heroPowerChoice6_Title",
      "heroPowerChoice7_Title",
      "heroPowerChoice8_Title",
      "heroPowerChoice9_Title",
      "heroPowerChoice10_Title",
      "heroPowerChoice11_Title",
      "heroPowerChoice1_Description",
      "heroPowerChoice2_Description",
      "heroPowerChoice3_Description",
      "heroPowerChoice4_Description",
      "heroPowerChoice5_Description",
      "heroPowerChoice6_Description",
      "heroPowerChoice7_Description",
      "heroPowerChoice8_Description",
      "heroPowerChoice9_Description",
      "heroPowerChoice10_Description",
      "heroPowerChoice11_Description",
      "changeCardTitle",
      "newCardButtonChange",
      "gameEndTitle",
      "gameEndButton",
      ...
      "formConfigGameMode",
      "formConfigWildMode",
      "formConfigStandardMode",
      "formConfigLastExpansionMode",
      "formConfigClassicMode",
      "formConfigArenaMode",
      "formConfigBattlegroundsMode",
      "formConfigFirstClue",
      "formConfigFlavorText",
    ...
    ["heroPower", "score", "goToConfigButton", "goToRulesButton"].forEach(
      function (elementId) {
        var element = document.getElementById(elementId);
    ...
    document.getElementById("rulesButton").innerHTML = translate(
      hasStartedGame ? "back" : "rulesButton"
    );

    document.getElementById("sampleStatsCost").title = translate(
      "statTitle_cost_ok",
      6
    );
    document.getElementById("sampleStatsAttack").title = translate(
      "statTitle_attack_almost",
      4
    );
    document.getElementById("sampleStatsHealth").title = translate(
      "statTitle_ko",
      9
    );
    ...
    switch (client.cardSetGroup) {
      case "battlegrounds":
        {
          cardMode = translate("formConfigBattlegroundsMode");
        }
        break;
      case "wild":
        {
          cardMode = translate("formConfigWildMode");
        }
        break;
      case "classic":
        {
          cardMode = translate("formConfigClassicMode");
        }
        break;
      case "arena":
        {
          cardMode = translate("formConfigArenaMode");
        }
        break;
      case "lastExpansion": {
        document.getElementById("guessCard").innerHTML = translate(
          "guessCardLastExpansion"
        );
        return;
      }
      default:
        {
          cardMode = translate("formConfigStandardMode");
        }
        break;
    }
  ...
  function onClientInit(client) {
    ...
    searchInput.oninput = function (e) {
      ...
      searchClock = window.setTimeout(function () {
        ...
        client.searchByCardName(nameToSeach).then(function (cards) {
          suggestions.innerHTML = "";
    ...
    document.getElementById("guessForm").onsubmit = function (e) {
      ...
      if (!correspondingCard) {
        showErrorModal(
          translate("cardNoFoundTitle"),
          translate("cardNoFoundContent")
        );
        return;
      }
      ...
      if (!isOK) {
        ...
        nameCell.style.backgroundImage =
          "url(" + correspondingCard.cropImage + ")";

        var nameContainer = document.createElement("div");
        nameContainer.className = "nameContainer hearthstoneText";
        nameContainer.appendChild(
          document.createTextNode(correspondingCard.name)
        );
        ...
        costCell.className =
          "tableCardStat cost hearthstoneText " +
          cardStatsStatus.cost +
          " value-" +
          cost;
        costCell.appendChild(document.createTextNode(cost));
        costCell.title = getStatTitle(
          "cost",
          cost,
          cardStatsStatus.cost,
          client.isBattlegrounds()
        );
        ...
        if ("attack" in correspondingCard) {
          attackClassName =
            correspondingCard.cardTypeId === HEARTHSTONE_CARD_TYPE_IDS.WEAPON
              ? "attack_weapon"
              : "attack";
        }
        ...
        healthCell.className =
          "tableCardStat " +
          healthType +
          " hearthstoneText " +
          cardStatsStatus.health;
        healthCell.appendChild(document.createTextNode(health));
        healthCell.title = getStatTitle(
          "health",
          health,
          cardStatsStatus.health
        );
        ...
        var successImageContainer =
          document.getElementById("roundSuccessImage");
        ...
        document.getElementById("hasBeenRestored").style.display =
          currentGame.hp === Game.MAX_HP ? "none" : "block";
    ...
    document.getElementById("heroPowerRarity").onclick = () =>
      useClue("RARITY", client);
    document.getElementById("heroPowerClass").onclick = () =>
      useClue("CLASS", client);
    document.getElementById("heroPowerExpansion").onclick = () =>
      useClue("EXPANSION", client);
    document.getElementById("heroPowerInitials").onclick = () =>
      useClue("INITIALS", client);
    document.getElementById("heroPowerChange").onclick = () =>
      useClue("CHANGE", client);
    document.getElementById("heroPowerFlavorText").onclick = () =>
      useClue("FLAVOR_TEXT", client);
    document.getElementById("heroPowerImage1").onclick = () =>
      useClue("IMAGE_1", client);
    document.getElementById("heroPowerImage2").onclick = () =>
      useClue("IMAGE_2", client);
    document.getElementById("heroPowerImage3").onclick = () =>
      useClue("IMAGE_3", client);
    document.getElementById("heroPowerType").onclick = () =>
      useClue("TYPE", client);
    document.getElementById("heroPowerText").onclick = () =>
      useClue("TEXT", client);

    document.getElementById("cancelHeroPowerButton").onclick = () =>
      closeHeroPowerModal();
